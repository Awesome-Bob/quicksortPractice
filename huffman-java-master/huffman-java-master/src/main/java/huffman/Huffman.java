...
public class Huffman {
    ...
    public static Map<Character, Integer> freqTable (String input) {
        ...
        if(input == null || input.isEmpty())return null;
            ...
            // put input in char array
            char[] strArray = input.toCharArray();
            //loop through array
            for(char c : strArray) {
                if (ft.containsKey(c)){
                    // if existing character in map is found increment frequency by 1
                    ft.put(c, ft.get(c) + 1);
                }
                // if character is not in map add to map and set frequency as 1
                else {
                    ft.put(c,1);
                }
            }
    ...
    public static Node  treeFromFreqTable (Map<Character, Integer> freqTable) {
        ...
        for (Map.Entry<Character, Integer> entry : freqTable.entrySet()) {
            ...
            p.enqueue(new Leaf(c,i));
        }
        // tree creator
        while (p.size() != 1){
            // deque two nodes to create branch node
            Node n = p.dequeue();
            Node m = p.dequeue();
            int nf=n.getFreq();
            int mf=m.getFreq();
            // node with smaller frequency set as left node
            if (nf<mf) {
                p.enqueue(new Branch(nf + mf, n, m));
            }else {
                p.enqueue(new Branch(nf + mf, m, n));
            }
        }
        // return tree as Node
        return p.dequeue();
    ...
    public static HuffmanCoding encode(String input) {
          buildCode(treeFromFreqTable(freqTable(input)));
